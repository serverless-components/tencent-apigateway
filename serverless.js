  ...
  async default(inputs = {}) {
    ...
    const apiAuthSetting = async (ctx, region, apiClient, endpoint) => {
      ...
      if (usagePlan.usagePlanId) {
        try {
          ...
          if (!CheckExistsFromError(e)) {
            this.context.debug(
              `UsagePlan ID ${usagePlan.usagePlanId} not found. Creating a new UsagePlan.`
            )
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (endpoint.auth) {
        ...
        this.context.debug(
          `Binding Secretkey ${result.secretIds.value} to UsagePlan with ID ${result.usagePlanId.value}.`
        )
    ...
    for (let i = 0; i < apisLen; i++) {
      ...
      if (endpoint.usagePlanId) {
        ...
        if (endpoint.usagePlanId.created == true) {
          this.context.debug(
            `Removing any previously deployed usagePlanIds ${endpoint.usagePlanId.value}`
          )
